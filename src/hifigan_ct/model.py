...
from src.hifigan_ct.env import AttrDict
from src.hifigan_ct.utils import init_weights, get_padding

LRELU_SLOPE = 0.1

def load_generator_from_path(model_path, device='cuda', return_hparams=False):
    config_file = os.path.join(os.path.split(model_path)[0], 'config.json')
    assert os.path.exists(config_file), 'config.json is missing from hifigan_ct path. Ensure config.json is in the same folder as the model/checkpoint.'
    ...
    if return_hparams:
        return model, h, cp_dict['hparams']
    else:
        return model, h
